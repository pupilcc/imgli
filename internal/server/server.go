...
func (s *Server) mountAPI() {
	...
	s.mux.Route("/api/v1", func(api chi.Router) {
		...
		api.With(limiter.IPMiddleware("plaza", 120)).Get("/plaza/albums", dh.PlazaAlbums)
		...
		api.With(limiter.IPMiddleware("plaza", 120)).Get("/u/{username}/albums", dh.UserAlbums)
		...
		api.With(limiter.IPMiddleware("public_album_unlock", 30)).Post("/a/{id}/unlock", albH.UnlockPublic)
		...
		api.Group(func(priv chi.Router) {
			...
			priv.Route("/albums", func(ar chi.Router) {
				...
				ar.Get("/{id}/stats", albH.Stats)
				ar.Post("/{id}/reorder", albH.Reorder)
				ar.Post("/{id}/images/visibility", albH.SetImagesVisibility)
