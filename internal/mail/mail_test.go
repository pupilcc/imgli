...
func TestTemplatesZH(t *testing.T) {
	sub, html := RenderResetPassword("白栗", "https://img.li/reset-password?token=abc", "zh")
	...
	// 空 lang 默认中文
	sub0, html0 := RenderResetPassword("白栗", "https://img.li/reset-password?token=abc", "")
	if !strings.Contains(sub0, "重置你的 白栗 密码") || !strings.Contains(html0, "重置密码") {
		t.Errorf("reset 空 lang 应中文: %q %q", sub0, html0)
	}
	sub2, html2 := RenderVerifyEmail("白栗", "https://img.li/verify-email?token=x", "zh")
...
func TestTemplatesEN(t *testing.T) {
	sub, html := RenderResetPassword("Imgli", "https://img.li/reset-password?token=abc", "en")
	...
	sub2, html2 := RenderVerifyEmail("Imgli", "https://img.li/verify-email?token=x", "en")
